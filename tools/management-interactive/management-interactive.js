...
const lessons={
 detective:{title:'管理偵探社',lead:'從忙碌的日常裡，找出真正的管理問題。',icon:'🔍',quote:'管理不是把事情做完，而是知道要去哪裡、如何配置資源，並確認是否真的產生結果。',qs:[
 ...
 direction:{title:'方向導航局',lead:'把漂亮的方向性語句，轉成真正可管理的目標。',icon:'🧭',quote:'方向不是寫得漂亮，而是讓團隊知道為什麼存在、往哪裡走，以及如何知道自己正在前進。',qs:[
 ...
 strategy:{title:'策略診療室',lead:'SWOT 不是終點；真正的策略發生在選擇與取捨。',icon:'♟️',quote:'Strategy = Choice。沒有取捨，就很難稱為策略。',qs:[
 ...
 leadership:{title:'主管的一天',lead:'在授權、支持與責任之間做出管理判斷。',icon:'👥',quote:'Give direction. Give authority. Keep accountability.',qs:[
 ...
 quality:{title:'品質改善實驗室',lead:'不要被漂亮數字騙了；先看資料能不能回答真正的問題。',icon:'📊',quote:'Data is not decoration.',qs:[
 ...
 systems:{title:'跨系統任務局',lead:'當每個人都完成自己的工作，服務仍可能在系統縫隙中掉下去。',icon:'🌐',quote:'The client does not live inside one system.',qs:[
...
const $=s=>document.querySelector(s);$('[data-title]').textContent=lesson.title;$('[data-lead]').textContent=lesson.lead;$('[data-kicker]').textContent=`AML MANAGEMENT · ${id.toUpperCase()}`;
function render(){const item=lesson.qs[i];answered=false;$('[data-scene]').textContent=`SCENE ${i+1}`;$('[data-story]').innerHTML=item.story.map(t=>`<div class="bubble">${t}</div>`).join('');$('[data-question]').textContent=item.q;$('[data-step]').textContent=`${i+1} / ${lesson.qs.length}`;$('[data-bar]').style.width=`${Math.round(i/lesson.qs.length*100)}%`;$('[data-feedback]').style.display='none';$('[data-next]').disabled=true;const opts=$('[data-options]');opts.innerHTML='';item.o.forEach((label,idx)=>{const b=document.createElement('button');b.className='opt';b.type='button';b.textContent=`${String.fromCharCode(65+idx)}. ${label}`;b.addEventListener('click',()=>choose(idx,b));opts.appendChild(b);});}
...
function done(){const s=load();s.games=s.games||{};s.games[id]=true;save(s);$('[data-comic]').style.display='none';$('[data-done]').style.display='block';$('[data-bar]').style.width='100%';$('[data-icon]').textContent=lesson.icon;$('[data-done-title]').textContent=`${lesson.title}｜挑戰完成`;$('[data-quote]').textContent=lesson.quote;$('[data-done-text]').textContent='已記錄本關完成狀態。回到自學路徑後，核心進度會自動更新。';}
